...
ElectronIDSelector::ElectronIDSelector(ElectronCollectionPtr input, float rho, VertexCollection vertices) : 
...
ElectronCollectionPtr ElectronIDSelector::output() {
...
ElectronCollectionPtr ElectronIDSelector::output_hlt() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
...
ElectronCollectionPtr ElectronIDSelector::output_cuts_id() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_cuts_iso() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_cuts_convrej() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_cuts_ip() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_cuts() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_mva_id() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_mva_iso() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_mva_convrej() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_mva_ip() {
  output_.clear();
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
    algo.setElectron(*(*ele));
...
ElectronCollectionPtr ElectronIDSelector::output_mva() {
  output_.clear();
  int iele=0;
  for(electronptr_iterator ele=input_.begin(); ele!=input_.end(); ++ele) {
    ElectronIDAlgo algo(rho_,vertices_);
